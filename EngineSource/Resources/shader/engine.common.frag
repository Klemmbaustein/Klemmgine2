...
vec2 lightStrength(vec3 at, float range, float falloff, float intensity, vec3 viewDir, float specularSize)
{
#define Kc 1.0
...
#define Kq (falloff * 1.5)
	...
	float spec_dir = pow(max(reflectDiff, 0.0), specularSize);
